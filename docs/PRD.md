...
| D18 | Accettazione umana sopra il gate (2026-09-17) | Un'ipotesi bocciata dal gate può essere accettata da un umano con merge manuale, un run di conferma committato e una riga `accepted (human)` nel changelog, separata dalla curva del loop. Prima applicazione: `hyp/19` | La regola del gate resta stretta (l'holdout non è un bersaglio); l'umano legge l'evidenza che il gate non ha, qui il testo identico di `F12` con flag diverso |
